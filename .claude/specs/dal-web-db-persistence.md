...
The current `Store` (`backend/app/services/store.py`) is a clean seam:
...
New package `backend/app/services/db/`:
...
- Env var `DAL_WEB_DB_URL` holds a SQLAlchemy URL. Default:
  `sqlite:///<repo>/backend/.data/dalweb.db`.
- `DAL_WEB_DB_URL=postgresql+psycopg://host/db ./scripts/start.sh`
...
`backend/pyproject.toml`: `sqlalchemy>=2.0`, `alembic`. Sync drivers only
...
- `backend/app/services/db/session.py`
- `backend/app/services/db/models.py`
- `backend/app/services/db/store_db.py`
- `backend/app/services/db/migrations/` (Alembic env + initial revision)
- `backend/alembic.ini`

Modified:

- `backend/app/services/store.py` — extract `StoreProtocol`, keep in-memory `Store`, route `get_store()`.
- `backend/app/main.py` — init DB on startup, idempotent seed.
...
2. With `DAL_WEB_DB_URL=sqlite:///<file>`, all five entity types survive a
   backend restart.
3. All existing `backend` tests pass unchanged.
