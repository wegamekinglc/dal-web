...
function curveViewValue(point: CurveLabCurveViewPoint, view: CurveView): string {
  ...
  return value === null ? "—" : `${value.toFixed(10)} (${(value * 100).toFixed(6)}%)`;
}
...
function calibrationTerms(
  ...
  if (instrumentType === "OIS" || instrumentType === "IRS") {
    return {
      ...
      index_name:
        instrumentType === "OIS" ? `${domesticCurrency}-SOFR` : `${domesticCurrency}-IBOR-3M`,
    };
  }
...
function familyCurrencyOrPair(current: string, instrumentType: CurveLabSuccessFamily): string {
...
function migrateCalibrationInstrument(
  ...
  const componentKey =
    previousTerms && typeof previousTerms === "object"
...
function AxisTable({ title, rows }: { title: string; rows: CurveLabBuildRun["quote_axis"] }) {
  return (
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={row.quote_id ?? row.parameter_id ?? index}>
                  <td {...css("mono", "num")}>
                    {row.global_quote_index ?? row.global_parameter_index}
                  </td>
...
function MatrixTable({ matrix }: { matrix: CurveLabMatrix }) {
  ...
  return (
    <section {...css("panel", "curve-lab-matrix")}>
      ...
      <p {...css("muted", "mono")}>
        {matrix.method} · {matrix.rows} × {matrix.columns}
      </p>
      {rows && (
        <div {...css("table-container")}>
          <table>
...
const CurveLabWorkspace = forwardRef<CurveLabWorkspaceHandle, CurveLabWorkspaceProps>(
  function CurveLabWorkspace({ onCanonicalTargetChange }, ref) {
    ...
    const [draftSource, setDraftSource] = useState(JSON.stringify(DEFAULT_DRAFT, null, 2));
    const [tradeSource, setTradeSource] = useState(JSON.stringify(DEFAULT_TRADES, null, 2));
    ...
    const addDeclaration = () => {
      ...
      updateVisualDraft((current) => ({
        ...
        instruments: [...visualInstruments, calibrationInstrument(componentKey, currency)],
      }));
    };
    const removeDeclaration = (index: number) => {
      ...
      updateVisualDraft((current) => ({
        ...
        instruments: visualInstruments.filter((instrument) => {
          const terms = instrument.terms;
          return (
            !terms ||
            typeof terms !== "object" ||
    ...
    const addInstrument = () => {
      const template = visualInstruments.slice(-1).pop() ?? DEFAULT_DRAFT.instruments[0];
      const next = {
    ...
    }, [canonicalTargetToken, onCanonicalTargetChange, selectedInstrumentFamily]);
    const applyCanonicalQuote = useCallback(
      (quote: CurveLabCanonicalQuote, expectedTargetToken: number): boolean => {
        if (
      ...
      [selectedInstrumentFamily, selectedInstrumentIndex, visualDraft, visualInstruments],
    );
    ...
    const pollBuild = async (initial: CurveLabBuildRun, options?: { switchToRuns?: boolean }) => {
      const terminal = await waitForTerminal(initial, api.getCurveLabBuildRun, setBuild);
    ...
    const saveDraft = () =>
      execute(async () => {
        ...
        setEditedAfterBuild(false);
        setStatus(
          `Saved draft ${updated.id.slice(0, 8)} revision ${updated.revision}; rebuild required.`,
        );
      });
    ...
    const pollRisk = async (initial: CurveLabRiskRun) => {
      const terminal = await waitForTerminal(initial, api.getCurveLabRiskRun, setRisk);
      const matrixIds = [
        "trade-to-node",
    ...
    const archive = (version: CurveLabVersion) =>
      execute(async () => {
    ...
    const clone = (version: CurveLabVersion) =>
      execute(async () => {
    ...
    const download = (version: CurveLabVersion) =>
      execute(async () => {
    ...
    const pollImport = async (initial: CurveLabImportJob) => {
      const terminal = await waitForTerminal(initial, api.getCurveLabImportJob, setImportJob);
      await refreshVersions();
    ...
    const activeDeclarationIndex =
      visualDeclarations.length === 0
        ? 0
    ...
    const fitState =
      build?.diagnostics && typeof build.diagnostics.fit_state === "string"
      ...
      <section {...css("curve-lab-v2")} aria-label="Curve Lab workflow">
        ...
        {tab === "build" && (
          <div {...css("curve-builder")}>
              ...
              <aside {...css("panel", "curve-builder-set")}>
                ...
                <div {...css("curve-builder-set-actions")}>
                  <dl {...css("curve-builder-ids")}>
                    <dt>Draft</dt>
                    <dd>
                      {draft ? `${draft.id.slice(0, 8)} · r${draft.revision}` : "not created"}
                    </dd>
                    <dt>Build</dt>
              ...
              <section {...css("curve-builder-instruments")}>
                  ...
                  <div {...css("table-container")}>
                    <table>
                      <thead>
                        <tr>
                          <th>
                            <span {...css("sr-only")}>Canonical target</span>
                          </th>
                          <th>Include</th>
                          <th>Type</th>
                          <th>Tenor</th>
                          ...
                          <th>Status</th>
                          <th>
                            <span {...css("sr-only")}>Row actions</span>
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td>
                                ...
                                >
                                  {CURVE_LAB_SUCCESS_FAMILIES.map((family) => (
                ...
                <details {...css("curve-builder-advanced")}>
                  ...
                  <p {...css("muted", "mono")}>
                    ...
                    {" · "}
                    {String(
                      (visualDraft.solver as Record<string, unknown> | undefined)
                        ?.parameterization ?? "PIECEWISE_CONSTANT_FWD",
                    )}
                  </p>
                  <label>
            ...
            <footer {...css("curve-builder-statusbar")}>
              ...
              <span>
                Dependencies: <strong>{dependencyVersionIds.length}</strong>
              </span>
              <span>
                Instruments: <strong>{visualInstruments.length}</strong>
              </span>
              <span>
                Status:{" "}
                <strong
                  {...css(
                    rebuildRequired ? "warn" : build?.state === "SUCCEEDED" ? "ok" : undefined,
                  )}
                >
              ...
              <span>
                Last success: <strong {...css("mono")}>{lastSuccess ?? "—"}</strong>
              </span>
            </footer>
          </div>
        )}
            ...
            {!build ? (
              ...
              <>
                <div {...css("cards", "result-cards")}>
                  <div {...css("card")}>
                    <h3>State</h3>
                    <div {...css("metric")}>{build.state}</div>
                  </div>
                  <div {...css("card")}>
                    <h3>Draft revision</h3>
                    <div {...css("metric")}>{build.draft_revision}</div>
                  </div>
                  <div {...css("card")}>
                    <h3>Quotes</h3>
                    <div {...css("metric")}>{build.quote_axis.length}</div>
                  </div>
                  <div {...css("card")}>
                    <h3>Parameters</h3>
                    <div {...css("metric")}>{build.parameter_axis.length}</div>
                  </div>
                </div>
                <p {...css("muted", "mono")}>
                ...
                <section {...css("panel")}>
                  ...
                  <p {...css("muted")}>
                    {curveView === "discount" &&
                      "Native discount factors from valuation date to each persisted node."}
                    {curveView === "zero" &&
                      "Continuously compounded ACT/365F zero rates on the persisted node axis."}
                    {curveView === "forward" &&
                      "One-day continuously compounded ACT/365F forwards; LEFT samples the preceding interval."}
                  </p>
                  <div {...css("table-container")}>
                        ...
                        {(build.curve_views ?? []).length === 0 && (
                          <tr>
                            <td colSpan={4} {...css("muted")}>
                              Numeric curve views are unavailable for this run. Rebuild the curve to
                              populate them.
                            </td>
                          </tr>
                        )}
          ...
          <div {...css("curve-lab-risk-layout")}>
                ...
                >
                  ...
                  {versions.map((version) => (
                    <option key={version.id} value={version.id}>
                      {version.name} · {version.id.slice(0, 8)}
                    </option>
                  ))}
                </select>
              </label>
            ...
            <aside {...css("panel", "curve-lab-actions")}>
              ...
              {risk && (
                <dl>
                  ...
                  <dd>{risk.state}</dd>
                  <dt>Prices</dt>
                  <dd>{risk.result?.pricing?.length ?? 0}</dd>
                  <dt>Evaluations</dt>
                  <dd>{String(risk.estimated_work.price_evaluations)}</dd>
                </dl>
              )}
                    ...
                    <tbody>
                      {risk.result.pricing.map((row, index) => (
                        <tr key={String(row.trade_id)}>
                          ...
                          <td {...css("mono", "num")}>
                            {String(itemAt(risk.result?.dv01, index)?.value ?? "—")}
                          </td>
                          <td {...css("mono", "num")}>
                            {String(itemAt(risk.result?.key_rate_sum, index)?.value ?? "—")}
                          </td>
                          <td {...css("error")}>
            ...
            {matrices.length > 0 && (
              <div {...css("matrix-grid", "curve-lab-matrices")}>
                {matrices.map((matrix) => (
                  <MatrixTable key={matrix.matrix_id} matrix={matrix} />
                ))}
              </div>
            )}
          </div>
        ...
        {tab === "versions" && (
          <section {...css("panel", "curve-lab-versions")}>
            <div {...css("curve-lab-section-heading")}>
              <div>
                <h3>Visible curve versions</h3>
                <p {...css("muted")}>Immutable native archives with explicit provenance.</p>
              </div>
              ...
              {selectedVersion && compareVersion ? (
                <table>
                  ...
                  <tbody>
                    <tr>
                      <td>Payload hash</td>
                      <td {...css("mono")}>{selectedVersion.native_payload_hash}</td>
                      <td {...css("mono")}>{compareVersion.native_payload_hash}</td>
                    </tr>
                    <tr>
                      <td>Source</td>
                      <td>{selectedVersion.source_kind}</td>
                      <td>{compareVersion.source_kind}</td>
                    </tr>
                    <tr>
                      <td>Root</td>
                      <td>{selectedVersion.root_kind}</td>
                      <td>{compareVersion.root_kind}</td>
                    </tr>
                    <tr>
                      <td>Validation</td>
                      <td>{selectedVersion.build_validation_state}</td>
                      <td>{compareVersion.build_validation_state}</td>
                    </tr>
                  </tbody>
                </table>
              ) : (
                  ...
                  {versions.map((version) => (
                    <tr key={version.id}>
                      <td>
                        <strong>{version.name}</strong>
                        <br />
                        <small {...css("mono")}>{version.id}</small>
                      </td>
                      <td>
                        <span {...css("tag")}>{version.source_kind}</span>
                      </td>
