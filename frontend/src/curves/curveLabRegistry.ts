...
export const CURVE_LAB_FAMILY_REGISTRY = [
  {
    ...
    coordinate: "RATE",
    canonicalRawUnit: "DECIMAL",
    inputConventions: ["DECIMAL", "PERCENT"],
  },
  {
    instrumentType: "FRA",
    coordinate: "RATE",
  ...
  },
  {
    ...
    coordinate: "PRICE",
    canonicalRawUnit: "PRICE_POINTS",
    inputConventions: ["PRICE_POINTS"],
  },
  {
    instrumentType: "OIS",
    coordinate: "RATE",
...
export function curveLabFamily(
  ...
  const result = CURVE_LAB_FAMILY_REGISTRY.find((row) => row.instrumentType === instrumentType);
  if (!result) {
