...
the [web application guide](../README.md). For installed native entry
points, see the [public API guide](https://github.com/wegamekinglc/Derivatives-Algorithms-Lib/blob/master/docs/public-api.md). Curve construction and
Jacobian methodology remain in
[yield_curve.md](https://github.com/wegamekinglc/Derivatives-Algorithms-Lib/blob/master/docs/methodology/yield_curve.md) and
[yield_curve_jacobian.md](https://github.com/wegamekinglc/Derivatives-Algorithms-Lib/blob/master/docs/methodology/yield_curve_jacobian.md).
...
`backend/openapi/dal-web.openapi.json`; regenerate it from
`backend` with:
...
Run the checks appropriate to the changed layer. Native build and binding
checks live in the DAL repository:
https://github.com/wegamekinglc/Derivatives-Algorithms-Lib — run
`build_linux.sh` and `ctest` from a DAL checkout, and see `dal-python/tests`
there for the binding tests.
...
`backend/openapi/dal-web.openapi.json` is unchanged unless the REST
